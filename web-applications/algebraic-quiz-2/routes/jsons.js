...
router.get('/quiz-list', function (req, res) {
    req.db.all("SELECT name, title, description, example, example_answer, example_punishments FROM quizzes;", function (err, quizzes) {
        checkInternalError(err, req, res);
        res.json(quizzes);
    });
});
router.get('/quiz', function (req, res, next) {
    req.db.get("SELECT questions_no, title, description FROM quizzes WHERE name = ?", [req.session.quiz], function (err, quizInfo) {
        checkInternalError(err, req, res);
        req.db.all("SELECT * FROM questions WHERE questions.quiz_name = ? ORDER BY question_no ASC", [req.session.quiz], function (err, questions) {
            checkInternalError(err, req, res);
...
router.get('/stats/:quizName', function (req, res) {
    ...
    req.db.all("SELECT * FROM results WHERE quiz_name = ?", [req.params.quizName], function (err, quizResults) {
        ...
        req.db.all("SELECT user_login, full_res FROM results WHERE quiz_name = ? ORDER BY full_res LIMIT 5;", [req.params.quizName], function (err, topUsers) {
            checkInternalError(err, req, res);
            resJson.topUsers = topUsers;
            req.db.get("SELECT correct_answers, punishment FROM quizzes WHERE name = ?", [req.params.quizName], function (err, selectedQuiz) {
                checkInternalError(err, req, res);
